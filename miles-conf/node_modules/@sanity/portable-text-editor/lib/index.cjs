...
var debug$g = debugWithName('operationToPatches');
// eslint-disable-next-line new-cap
...
function createPatchToOperations(portableTextFeatures, keyGenerator) {
    function diffMatchPatch(editor, patch) {
        ...
        if (parsed && editor.isTextBlock(block)) {
            ...
            debug$g("DiffMatchPatch (".concat(distance < 0 ? 'remove' : 'insert', ") at ").concat(JSON.stringify(slatePath), "}: "), JSON.stringify(point, null, 2), JSON.stringify(parsed, null, 2));
    ...
    function insertPatch(editor, patch) {
        ...
        if (patch.path.length === 1) {
            ...
            debug$g("Inserting blocks at path [".concat(normalizedIdx_1, "]"));
            debugState(editor, 'before');
        ...
        debug$g("Inserting children at path ".concat(targetPath));
        debugState(editor, 'before');
    ...
    function setPatch(editor, patch) {
        ...
        if (isTextBlock) {
            debug$g("Setting nodes at ".concat(JSON.stringify(patch.path), " - ").concat(JSON.stringify(targetPath)));
            debug$g('Value to set', JSON.stringify(value, null, 2));
            debugState(editor, 'before');
            if (targetPath.length === 1) {
                ...
                //   patch.path[3] === 'text'
                if (onSamePath) {
                    debug$g('On same path, restoring previous selection');
    ...
    function unsetPatch(editor, patch, previousPatch) {
        ...
        if (patch.path.length === 1) {
            ...
            debug$g("Removing block at path [".concat(index, "]"));
            debugState(editor, 'before');
            if (editor.selection &&
        ...
        if (isTextBlock && patch.path[1] === 'children' && patch.path.length === 3) {
            ...
            debug$g("Removing child at path ".concat(JSON.stringify(targetPath)));
            debugState(editor, 'before');
            if (prevSel && onSamePath && editor.isTextBlock(block)) {
                ...
                if (needToAdjust) {
                    ...
                    if (isMergeUnset) {
                        debug$g('Adjusting selection for merging of nodes');
...
var debug$f = debugWithName('plugin:withUndoRedo');
// eslint-disable-next-line new-cap
...
function transformOperation(editor, patch, operation) {
    ...
    if (patch.type === 'diffMatchPatch') {
        ...
        if (slate.Element.isElement(block) && Array.isArray(block.children)) {
            ...
            if (!parsed) {
                debug$f('Could not parse diffMatchPatch', patch);
                return [operation];
            }
            var distance = parsed.length2 - parsed.length1;
...
function adjustBlockPath(editor, patch, operation, level) {
    ...
    if (myIndex >= 0 &&
        ...
        debug$f("Adjusting ".concat(operation.type, " for block ").concat(patch.type), operation.path, newPath);
...
var debug$e = debugWithName('plugin:withPatches');
    ...
    return [
        function withPatches(editor) {
                                    ...
                                    withoutSaving(editor, function () {
                                        withPreserveKeys(editor, function () {
                                            try {
                                                ...
                                                debug$e('Got error trying to create operations from patch');
                                                console.error(err);
                                            }
                                        });
                                    });
            ...
            editor.apply = function (operation) {
                ...
                if (!isPatching(editor)) {
                    debug$e("Editor is not producing patch for operation ".concat(operation.type), operation);
                    return editor;
                }
...
var debug$d = debugWithName('plugin:withPlaceholderBlock');
/**
    ...
    return function withPlaceholderBlock(editor) {
        ...
        editor.onChange = function () {
            ...
            if (editor.children.length === 0) {
                withoutPatching(editor, function () {
                    withoutSaving(editor, function () {
                        debug$d('Inserting placeholder block');
                        slate.Transforms.deselect(editor);
...
var debug$c = debugWithName('plugin:withPortableTextBlockStyle');
...
var debug$b = debugWithName('plugin:withPortableTextLists');
...
/* eslint-disable complexity */
var debug$a = debugWithName('plugin:withPortableTextMarkModel');
function createWithPortableTextMarkModel(portableTextFeatures, keyGenerator) {
    return function withPortableTextMarkModel(editor) {
        ...
        editor.pteToggleMark = function (mark) {
            ...
            if (isActive) {
                debug$a("Remove mark '".concat(mark, "'"));
                slate.Editor.removeMark(editor, mark);
            }
            else {
                debug$a("Add mark '".concat(mark, "'"));
...
var debug$9 = debugWithName('plugin:withPortableTextSelections');
...
function createWithPortableTextSelections(change$, portableTextFeatures) {
    return function withPortableTextSelections(editor) {
        var emitPortableTextSelection = function () {
            ...
            debug$9("Emitting selection ".concat(JSON.stringify(ptRange || null)));
            if (ptRange) {
...
var debug$8 = debugWithName('plugin:withSchemaTypes');
/**
...
var debug$7 = debugWithName('plugin:withUtils');
/**
...
var debug$6 = debugWithName('API:editable');
...
var withPlugins = function (editor, options) {
    ...
    var portableTextFeatures = options.portableTextFeatures, keyGenerator = options.keyGenerator, change$ = options.change$, incomingPatches$ = options.incomingPatches$, syncValue = options.syncValue;
    ...
    var _a = __read(options.readOnly
        ? []
        : createWithPatches({
    ...
    var _b = __read(options.readOnly
        ? []
    ...
    if (options.readOnly) {
        return withSchemaTypes(withObjectKeys(withPortableTextMarkModel(withPortableTextBlockStyle(withUtils(withPlaceholderBlock(withPortableTextLists(e)))))));
    }
    // The 'if' here is only to satisfy Typscript
    if (withUndoRedo && withPatches && withPortableTextSelections) {
        // Ordering is important here, selection dealing last, data manipulation in the middle and core model stuff first.
        return withSchemaTypes(withObjectKeys(withPortableTextMarkModel(withPortableTextBlockStyle(withPortableTextLists(withPlaceholderBlock(withUtils(withMaxBlocks(withUndoRedo(withPatches(withPortableTextSelections(e)))))))))));
    }
    return e;
};
...
var PortableTextEditor = /** @class */ (function (_super) {
    ...
    function PortableTextEditor(props) {
        ...
        _this.slateInstance = withPlugins(slateReact.withReact(slate.createEditor()), {
            change$: _this.change$,
            incomingPatches$: _this.incomingPatches$,
            keyGenerator: _this.keyGenerator,
            maxBlocks: _this.maxBlocks,
            portableTextFeatures: _this.portableTextFeatures,
            readOnly: _this.readOnly,
            syncValue: _this.syncValue,
        });
...
var PortableTextEditable = React.forwardRef(function PortableTextEditable(props, forwardedRef) {
    ...
    var withEditableAPI = React.useMemo(function () { return createWithEditableAPI(portableTextEditor, portableTextFeatures, keyGenerator); }, [keyGenerator, portableTextEditor, portableTextFeatures]);
